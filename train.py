...
def model_fn(batch, model, criterion, self_exclude, ref_included, device):
    ...
    srcs, src_masks, refs, ref_masks, tgts, tgt_masks, tgt_lens, stop_tokens = batch
    ...
    outs, out_posts, attn, attns, stop_preds = model(
        srcs, refs, tgts, src_masks=src_masks, ref_masks=ref_masks)

    losses, post_losses, st_losses = [], [], []
    for out, tgt, tgt_len in zip(outs.unbind(), tgts.unbind(), tgt_lens):
        loss = criterion(out[:, :tgt_len], tgt[:, :tgt_len])
        losses.append(loss)

    for out_post, tgt, tgt_len in zip(out_posts.unbind(), tgts.unbind(), tgt_lens):
        post_loss = criterion(out_post[:, :tgt_len], tgt[:, :tgt_len])
        post_losses.append(post_loss)

    for stop_pred, stop_token, tgt_len in zip(
        stop_preds.unbind(), stop_tokens.unbind(), tgt_lens
    ):
        st_loss = BCEWithLogitsLoss(stop_pred[:tgt_len], stop_token[:tgt_len])
        st_losses.append(st_loss)

    loss = sum(losses) / len(losses)
    post_loss = sum(post_losses) / len(post_losses)
    st_loss = sum(st_losses) / len(st_losses)

    return loss, post_loss, st_loss, [attn] + attns
...
def valid(dataloader, model, criterion, device):
    ...
    model.eval()
    running_loss, running_post_loss, running_st_loss = 0.0, 0.0, 0.0
    pbar = tqdm(total=len(dataloader.dataset),
                ncols=0, desc="Valid", unit=" uttr")

    for i, batch in enumerate(dataloader):
        with torch.no_grad():
            loss, post_loss, se_loss, _ = model_fn(
                batch, model, criterion, 1.0, True, device
            )
            running_loss += loss.item()
            running_post_loss += post_loss.item()
            running_st_loss += se_loss.item()

        pbar.update(dataloader.batch_size)
        pbar.set_postfix(
            loss=f"{running_loss / (i+1):.2f}, {running_post_loss / (i+1):.2f}, {running_st_loss / (i+1):.2f}"
        )
    ...
    return (
        ...
        running_post_loss / len(dataloader),
        running_st_loss / len(dataloader),
    )
...
def main(
    ...
    for step in range(total_steps):
        batch_loss, batch_post_loss, batch_st_loss = 0.0, 0.0, 0.0
        ...
        for _ in range(accu_steps):
            ...
            loss, post_loss, st_loss, attns = model_fn(
                batch, model, criterion, self_exclude, ref_included, device)
            total_loss = (loss + post_loss + st_loss) / accu_steps
            total_loss.backward()

            batch_loss += loss.item() / accu_steps
            batch_post_loss += post_loss.item() / accu_steps
            batch_st_loss += st_loss.item() / accu_steps
        ...
        pbar.update()
        pbar.set_postfix(
            loss=f"{batch_loss:.2f}, {batch_post_loss:.2f}, {batch_st_loss:.2f}",
            excl=self_exclude,
            step=step + 1,
        )
        ...
        if step % log_steps == 0 and comment is not None:
            ...
            writer.add_scalar("Post_Loss/train", batch_post_loss, step)
            writer.add_scalar("ST_Loss/train", batch_st_loss, step)
            writer.add_scalar("Self-exclusive Rate", self_exclude, step)

        if (step + 1) % valid_steps == 0:
            pbar.close()

            valid_loss, valid_post_loss, valid_st_loss = valid(
                valid_loader, model, criterion, device
            )

            if comment is not None:
                writer.add_scalar("Loss/valid", valid_loss, step + 1)
                writer.add_scalar("Post_Loss/valid", valid_post_loss, step + 1)
                writer.add_scalar("ST_Loss/valid", valid_st_loss, step)
                fig, axes = plt.subplots(
                    len(attns), 1, squeeze=False, figsize=(10, 10))

                for i, attn in enumerate(attns):
                    axes[i][0].imshow(
                        attn[0].detach().cpu().numpy(), origin="lower")
                    axes[i][0].set_title(
                        "layer {}".format(i), fontsize="medium")
                    axes[i][0].tick_params(labelsize="x-small")
                    axes[i][0].set_xlabel("target")
                    axes[i][0].set_ylabel("source")

                plt.tight_layout()
                writer.add_figure("Attention", fig, step + 1)
                plt.close()
        ...
        if (step + 1) >= milestones[1]:
            ...
            self_exclude = (
                step + 1 - milestones[0]) / (milestones[1] - milestones[0])
            self_exclude *= exclusive_rate
