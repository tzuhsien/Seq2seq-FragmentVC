...
def collate_batch(batch):
    ...
    stop_tokens = [torch.arange(tgts.size(2)) >= (tgt_len - 1)
                   for tgt_len in tgt_lens]
    stop_tokens = torch.stack(stop_tokens).float()

    return srcs, src_masks, refs, ref_masks, tgts, tgt_masks, tgt_lens, stop_tokens
